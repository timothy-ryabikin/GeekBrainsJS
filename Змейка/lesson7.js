...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        var scoreText = "Счёт: ";
        scoreText += score;
        scoreBlock.innerHTML = scoreText;
    }
    return check;
}
...
function createBomb() {
    ...
    while (!BombCreated) { //пока не создали бомбу
        // рандом
        ...
        if ((!bomb_cell_classes.includes('snake-unit')) || (!bomb_cell_classes.includes('food-unit'))) {
            ...
            massOfBombs.push(bomb_cell);
            //Если бомб больше, чем надо - удаляем одну рандомную
            if (massOfBombs.length >= 6) {
                var deleteID = Math.floor(Math.random() * massOfBombs.length);
                var deleteBomb = massOfBombs[deleteID];
                deleteBomb.classList.toggle("bomb-unit");
                massOfBombs.splice(deleteID, 1);
            }
            BombCreated = true;
        }
    }
